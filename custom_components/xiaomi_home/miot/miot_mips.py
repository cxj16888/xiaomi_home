...
class MipsClient(ABC):
    ...
    def sub_mips_state(
        ...
        self._internal_loop.call_soon_threadsafe(
            self.__sub_mips_state, key, handler)
        return True
    ...
    def unsub_mips_state(self, key: str) -> bool:
        ...
        if isinstance(key, str) is False:
            raise MIoTMipsError('invalid params')
        self._internal_loop.call_soon_threadsafe(self.__unsub_mips_state, key)
        return True
    ...
    def __on_connect_failed(self, client:Client, user_data:Any) -> None:
        self.log_error(f'mips connect failed')
        # Try to reconnect
...
class MipsLocalClient(MipsClient):
    ...
    @final
    @property
    def on_dev_list_changed(self) -> Callable[[Any, list[str]], Coroutine] | None:
    ...
    def _on_mips_message(self, topic: str, payload: bytes) -> None:
        ...
        if topic == self._reply_topic:
            ...
            req: MipsRequest | None = self._request_map.pop(str(mips_msg.mid), None)
            if req:
                # Cancel timer
                if req.timer:
                    req.timer.cancel()
